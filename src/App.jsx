  ...
  return <main className="site-shell">
    ...
    {view === 'tarot' && !stageOpen && <div className="app-grid">
      ...
      <MagicPanel className="template-panel">
        ...
        <div className="prompt-list">{(scenarioQuestionGroups[category] ? scenarioQuestionGroups[category][scene] ?? [] : flatQuestionGroups[category]).map((item) => <button type="button" className="beam-question" disabled={loading || isChoosingCards} onClick={() => { setQuestion(item); questionRef.current?.focus(); }} key={item}><span className="beam-question__copy">{item}</span><span className="beam-question__icon" aria-hidden="true">↗</span></button>)}</div>
