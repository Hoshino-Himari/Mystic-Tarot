...
## 無牌陣（free-1 / free-3 / free-5）

依站主提供的教學加入：牌按抽出順序排列（第 N 張），沒有位置意義，最多五張。
GPT 提示詞走獨立模板 `freeFormPrompt`——單抽時「這張牌就是解答，逆位拆成
預測＋回正位建議」；多張時用四原則：幻燈片式串故事、正逆位比例找阻礙、
大牌當主軸、四元素分布看特質。不要把無牌陣改成有位置意義的牌陣。

## 占卜模式（readingModes）

一般／高我對話／寵物占卜／投資指引／尋物。前四種共用牌陣流程，只透過
`modeDirectives` 在 GPT 提示詞注入視角（高我口吻、毛孩心聲、投資反思＋
免責聲明）。**尋物**是特殊模式：隱藏牌陣選擇、固定從 16 張宮廷牌抽 1 張
（`lostSpread`），翻牌後顯示 `LostItemResult` 方位盒——牌組元素（權杖火南／
聖杯水西／寶劍風東／錢幣土北）×位階元素（國王火／皇后水／騎士風／侍者土）
取方位交集；同向＝正方位、相鄰＝斜方位、對衝＝找原地附近與高低差。
投資指引的提示詞必須保留「非投資建議、盈虧自負」聲明，不要拿掉。
